...
namespace PssFramework.SpriteEngine
{
	public struct Vector2i
	{
		...
		{
			return new Vector2i( Common.Min( X, value.X ), Common.Min( Y, value.Y ) );
		}

		/// <summary>Element wise clamp.</summary>
		public Vector2i Clamp( Vector2i min, Vector2i max )
		{
			return new Vector2i( Common.Clamp( X, min.X, max.X ), Common.Clamp( Y, min.Y, max.Y ) );
		}
		...
		{
			return new Vector2i( Common.ClampIndex( X, n.X ), Common.ClampIndex( Y, n.Y ) );
		}
		...
		{
			return new Vector2i( Common.WrapIndex( X, n.X ), Common.WrapIndex( Y, n.Y ) );
		}
